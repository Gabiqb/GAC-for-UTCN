...
void initClipWindow()
{
	...
	clipWindow2.push_back(vec3(655.0f, 200.0f, 1.0f));
	clipWindow2.push_back(vec3(1005.0f,200.0f, 1.0f));
	clipWindow2.push_back(vec3(1100.0f,400.0f, 1.0f));
	clipWindow2.push_back(vec3(755.0f, 500.0f, 1.0f));
	clipWindow2.push_back(vec3(689.0f, 500.0f, 1.0f));
	clipWindow2.push_back(vec3(655.0f, 500.0f, 1.0f));
}
		...
		if (SDL_WaitEvent(&currentEvent) != 0)
		{
			...
			if (currentEvent.type == SDL_KEYDOWN)
			{
				...
				case SDLK_d:
					...
					//Applies the Cohen-Sutherland clipping algorithm -> implemented by you
					if (lineClip_CyrusBeck(clipWindow2, p1, p2) == -1)
						p1.x = p2.x = p1.y = p2.y = 0.0f;
					break;
				case SDLK_ESCAPE:
					quit = true;
			...
			for (size_t i = 0; i < clipWindow2.size(); i++)
				SDL_RenderDrawLine(
					windowRenderer,
					(int)clipWindow2.at(i).x,
					(int)clipWindow2.at(i).y,
					(int)clipWindow2.at((i + 1) % clipWindow2.size()).x,
					(int)clipWindow2.at((i + 1) % clipWindow2.size()).y);
			//draw the line
